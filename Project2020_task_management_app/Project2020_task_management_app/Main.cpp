...
void timePrint(void) {
	...
	while (!isEnd) {
		...
		gotoxy(12, 2);
		printf("%2d:%02d:%02d", curTime->tm_hour, curTime->tm_min, curTime->tm_sec);
		Sleep(500);
	}
}
...
int calendar(void) {
	...
	do {
		...
		design_calendar();
		gotoxy(1, 1);
		printf(" %d년 - %d월 - %d일", mt.pLocal->tm_year + 1900, mt.pLocal->tm_mon + 1, mt.pLocal->tm_mday);
		gotoxy(1, 7);
		textcolor(4, 0);
		printf("\t일");
		textcolor(7, 0);
		...
		for (int i = 0; i < 5; i++) {
			...
			for (int j = 0; j < 7; j++) {
				if (mt.date[i][j] != 0) {
					if (j == 0) textcolor(4, 0);
					if (j == 6)textcolor(1, 0);
					if(mt.date[i][j] == mt.pLocal->tm_mday) textcolor(2, 0);
